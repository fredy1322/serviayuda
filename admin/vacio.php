...
$sqlcuentach3 = "SELECT * FROM vacio WHERE estado='visto'";
if ($resultach3=mysqli_query($conexion,$sqlcuentach3)) {
    $rowcountch3=mysqli_num_rows($resultach3);
    if(empty($rowcountch3)) { $ocul2 = "hidden";} else { $ocul2 = "";}
}
...
&nbsp;
  ...
  <form method="POST">
<input type="submit" class="btn btn-dark" value="Ver solicitudes ocultas" name="ocultas">
</form>
<?php 

if(isset($_POST['ocultas'])){ $ocul = "hidden"; ?>

<!-- Muestra tabla con solicitudes ocultas -->
<h2 class="text-black" style="text-align: center; padding-bottom: 3%;">Solicitudes ocultas</h2>

<table class="tablahorarios" <?php echo $ocul2; ?>>
<thead>
  <tr>
  ...
  <tbody>
      <?php
                $sentencia="SELECT * FROM vacio WHERE estado ='visto'";
                $resultado = $conexion->query($sentencia) or die (mysqli_error($conexion));
                      while ($filas = mysqli_fetch_array($resultado)) {
                        
                        $fechai = $filas['fechaingreso']; 
                        $fechas = $filas['fecha']; 
                        $fechasolicitud = $filas['fechadesolicitud']; 
                        $fechai2 = date("d-m-Y", strtotime($fechai));
                        $fechai3 = date("d-m-Y", strtotime($fechas));
                        $fechads = date("d-m-Y", strtotime($fechasolicitud));
...
<td><?php echo $fechai2; ?></td>
<td><?php echo $fechai3; ?></td>
<td><?php echo $fechads; ?></td>
...
<?php if(empty($rowcountch2)) { ?>
   <h4 <?php echo $ocul; ?>> No cuenta con solicitudes pendientes de vacio laboral
...
<?php } else { ?>
  ...
  <h2 <?php echo $ocul; ?> class="text-black" style="text-align: center; padding-bottom: 3%;">Solicitud de vacio laboral</h2>
          
<!-- CODIGO NUEVO -->

<table class="tablahorarios" <?php echo $ocul; ?>>
<thead>
  <tr>
      <th> Nombre solicitante </th>
      <th> Cedula  </th>
      <th> Fecha de ingreso </th>
      <th> Fecha solicitada </th>
      <th title="Cantidad de dias entre la fecha que debe salir a vacio y la fecha que el agente solicitó."> Diferencia* </th>
      <th> Acciones </th>
      <th>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
  </tr>
</thead>
  <tbody>
      <?php
                $sentencia="SELECT * FROM vacio WHERE estado ='activo'";
                $resultado = $conexion->query($sentencia) or die (mysqli_error($conexion));
                      while ($filas = mysqli_fetch_array($resultado)) {
                        ...
                        $fecha1= new DateTime($fechai."+ 1 year");
...
echo $diff->days . ' dias';
?> </td>
<td>  <a class="fcc-btn" href="/Login/cambiohorario/acciones/quitarvacio.php?id=<?php echo $filas['id']; ?>"> <button class="btn btn-success"> Ocultar </button></a> </td>
